...
CCamera::CCamera(GLubyte bCamNum)
{
	...
	else if (bCamNum == CAMERA_ORTHO)
	{
		float terrainWidth = 513;
		float terrainDepth = 513;
		float terrainCenterX = terrainWidth * 0.5f;
		float terrainCenterZ = terrainDepth * 0.5f;

		const SVector3Df v3Pos = SVector3Df(terrainCenterX, 500.0f, terrainCenterZ);
		const SVector3Df v3Target = SVector3Df(terrainCenterX, 0.0f, terrainCenterZ);
		const SVector3Df v3Up = SVector3Df(0.0f, 0.0f, 1.0f);

		TOrthoProjInfo orthoProj;
		orthoProj.fLeft = 0.0f;
		orthoProj.fRight = terrainWidth;
		orthoProj.fBottom = 0.0f;
		orthoProj.fTop = terrainDepth;
		orthoProj.fNearZ = -1000.0f;
		orthoProj.fFarZ = 1000.0f;
		orthoProj.fWidth = terrainWidth;
		orthoProj.fHeight = terrainDepth;
...
bool CCamera::IsSprinting() const
{
	return (m_bIsSprinting);
}

void CCamera::SetSprinting()
{
	if (IsSprinting())
	{
		m_bIsSprinting = false;
...
CCameraManager::CCameraManager()
{
	...
	AddCamera(CAMERA_ORTHO);


	SetCurrentCamera(CAMERA_PRESPECTIVE);
}

bool CCameraManager::AddCamera(GLubyte CamNum)
{
	...
	m_mCamMap.insert(TCameraMap::value_type(CamNum, new CCamera(CamNum)));
	return true;
}

bool CCameraManager::RemoveCamera(GLubyte CamNum)
{
...
void CCameraManager::SetCurrentCamera(GLubyte CamNum)
{
...
void CCameraManager::ResetToPreviousCamera()
{
	if (!m_pPrevCam)
	{
		sys_err("CCameraManager::ResetToPreviousCamera: No Previous Camera Found! Keep the current");
		return;
	}

	m_pCurrCamera = m_pPrevCam;
	m_pPrevCam = nullptr;
}

bool CCameraManager::IsCurrentCamera(const GLubyte CamNum)
{
	if (m_pCurrCamera == m_mCamMap[CamNum])
	{
		return (true);
