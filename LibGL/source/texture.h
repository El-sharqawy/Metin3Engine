...
public:
	...
	CTexture& operator=(CTexture&& other) noexcept
	{
		if (this != &other)
		{
			...
			// Transfer resources
			m_uiTextureID = other.m_uiTextureID;
			m_bImageData = std::move(other.m_bImageData);
			m_iWidth = other.m_iWidth;
			m_iHeight = other.m_iHeight;
			m_iChannelsBPP = other.m_iChannelsBPP;
			m_eTextureTarget = other.m_eTextureTarget;
	...
	const std::string& GetTextureFullName() const;
	...
	GLuint GenerateAlphaTexture(const std::vector<std::vector<GLubyte>>& vvbMipLevels, GLsizei iWidth, GLsizei iHeight);
	GLuint Generate();
...
private:
	...
	GLint m_iMipLevels;
	unsigned char* m_bImageData; // Stores raw pixel data
