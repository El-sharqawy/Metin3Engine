...
CSkyBox::CSkyBox(CWindow* pWindow)
{
	...
	// Light Data
	m_v3LightDir = SVector3Df(-0.5f, 0.5f, 1.0f);
	m_v3LightDir.normalize();
	m_v3LightPos = m_v3LightDir * 1e6f + CCameraManager::Instance().GetCurrentCamera()->GetPosition();
	m_v3FogColor = SVector3Df(0.5f, 0.6f, 0.7f);
...
void CSkyBox::Render(const CCamera& rCamera)
{
    ...
    m_pSkyboxScreenSpace->GetShader().setMat4("m4Inv_proj", rCamera.GetProjectionMat().Inverse());
    m_pSkyboxScreenSpace->GetShader().setMat4("m4Inv_view", rCamera.GetViewMatrix().Inverse());
	...
	SVector3Df v3LightDir = (m_v3LightPos);// - CCameraManager::Instance().GetCurrentCamera()->GetPosition());
...
void CSkyBox::MixSkyColorPresets(GLfloat fVal, const TColorPreset& p1, const TColorPreset& p2)
{
	...
	m_v3LightColor = p1.v3LightColor * fA + p2.v3LightColor * fFactor;
