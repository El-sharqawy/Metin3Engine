...
void CMatrix4Df::InitLookAt(const SVector3Df& pos, const SVector3Df& target, const SVector3Df& up)
{
	SVector3Df zAxis = (pos - target).normalize();         // Forward (camera looks toward -Z)
	SVector3Df xAxis = up.cross(zAxis).normalize();        // Right
	SVector3Df yAxis = zAxis.cross(xAxis);                 // Up
	...
	mat4[3][0] = -xAxis.dot(pos);
	mat4[3][1] = -yAxis.dot(pos);
	mat4[3][2] = -zAxis.dot(pos);
	mat4[3][3] = 1.0f;
}
