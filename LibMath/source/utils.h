...
extern void Vec3Lerp(SVector3Df& v3CurPos, const SVector3Df& v3Start, const SVector3Df& v3End, float t);

namespace MyMath
{
    inline int imin(int iMin, int iMax)
    {
        return (iMin < iMax) ? iMin : iMax;
    }

    inline int imax(int iMin, int iMax)
    {
        return (iMax > iMin) ? iMax : iMin;
    }

    inline int iminmax(int iMin, int iVal, int iMax)
    {
        int fNewVal = (iMin > iVal) ? iMin : iVal;
        return (iMax < fNewVal) ? iMax : fNewVal;
    }
