...
uniform float distorsionStrength = 0.04f;
uniform float specularPower = 20.0f;
...
void main()
{
	...
	distortedTexCoords = v2TexCoord + vec2(distortedTexCoords.x, distortedTexCoords.y + u_fMoveFactor);
	vec2 totalDistortion = (texture(DUDVMapTexture, distortedTexCoords).rg * 2.0 - 1.0) * u_fMoveFactor * clamp(waterDepth / 20.0, 0.0, 1.0);
	...
	vec3 reflectedLight = reflect(normalize(v3WorldPos - v3LightPos), normal);
	...
	// Replace refractiveFactor calc with proper Fresnel logic
	float fresnel = dot(viewVector, normal);
	fresnel = clamp(fresnel, 0.0, 1.0);
	fresnel = 1.0 - fresnel; // Invert so that reflection is stronger at glancing angles
	fresnel = pow(fresnel, 4.0); // Increase power for sharper falloff (try 3.0 to 5.0)

	// Blend reflection and refraction with fresnel
	vec3 finalColor = mix(refractColor.rgb, reflectColor.rgb, fresnel);

	// Optional: Water color tint (absorption)
	finalColor = mix(finalColor, vec3(0.0, 0.3, 0.8), 0.2);

	// Add specular highlights
	finalColor += specularHighlights;

	FragColor = vec4(finalColor, v4Color.a);
